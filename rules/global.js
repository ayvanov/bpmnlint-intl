...
module.exports = function () {
  function check(node, reporter) {
    ...
    rootElements.forEach((rootElement) => {
      if (!hasName(rootElement)) {
        reporter.report(rootElement.id, t("global.missingName"));
      }

      if (!isReferenced(rootElement, referencingElements)) {
        reporter.report(rootElement.id, t("global.unused"));
      }

      if (!isUnique(rootElement, rootElements)) {
        reporter.report(rootElement.id, t("global.nameNotUnique"));
      }
    });
  }
  ...
  function getReferencingElements(definitions) {
    ...
    function traverse(element) {
      ...
      if (
        is(element, "bpmn:FlowElementsContainer") &&
  ...
  function isReferenced(rootElement, referencingElements) {
    if (is(rootElement, "bpmn:Error")) {
      return referencingElements.some((referencingElement) => {
        return (
          is(referencingElement, "bpmn:ErrorEventDefinition") &&
          rootElement.get("id") ===
    ...
    if (is(rootElement, "bpmn:Escalation")) {
      return referencingElements.some((referencingElement) => {
        return (
          is(referencingElement, "bpmn:EscalationEventDefinition") &&
          rootElement.get("id") ===
    ...
    if (is(rootElement, "bpmn:Signal")) {
      return referencingElements.some((referencingElement) => {
        return (
          is(referencingElement, "bpmn:SignalEventDefinition") &&
          rootElement.get("id") ===
  ...
  function isUnique(rootElement, rootElements) {
    return (
      rootElements.filter(
        (otherRootElement) =>
          is(otherRootElement, rootElement.$type) &&
