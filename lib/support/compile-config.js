...
import NodeResolver from "../resolver/node-resolver.js";
...
`;
  ...
  const ruleImportCode = Object.entries(rules)
    .map(([key, value], idx) => {
      if (!value) {
        return null;
      }

      const { pkg, ruleName } = linter.parseRuleName(key);

      return createRuleImportStatement(pkg, ruleName, idx, resolver);
    })
    .filter((e) => e)
    .join("\n");

  const moddleExtensionsImportCode = Object.entries(
    config.moddleExtensions || {},
  )
    .map(([key, importPath], idx) => {
      if (typeof importPath !== "string") {
        throw new Error(
          `illegal value for moddle-extension ${key}: expected <string>`,
        );
      }

      return createModdleExtensionStatement(key, importPath, idx);
    })
    .filter((e) => e)
    .join("\n");

  return `${preambleCode}${ruleImportCode}\n${moddleExtensionsImportCode}`;
}
...
function createRuleImportStatement(pkg, ruleName, idx, resolver) {
 ...
 * @return {string}
 */
function createModdleExtensionStatement(key, importPath, idx) {
  // (2) resolve rule via $PKG/rules/$NAME
  return `
import moddle_extension_${idx} from '${importPath}';

moddleExtensions['${key}'] = moddle_extension_${idx};`;
}
...
function computeRuleImport(resolver, pkg, originalPkg, rule) {
  ...
  if (rule.startsWith(".")) {
    ...
    return path.posix.normalize(
      `${originalPkg}/${relativePkgMainPath}/${rule}`,
    );
  } else {
    // absolute reference
