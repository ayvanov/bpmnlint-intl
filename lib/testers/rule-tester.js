...
export function expectEqual(a, b) {
  assert.deepStrictEqual(
    JSON.stringify(a, replacer, 2),
    JSON.stringify(b, replacer, 2),
  );
}

export function verify(ruleName, rule, testCases) {
  ...
  describe(`rules/${ruleName}`, function () {
    ...
    describe("should lint valid", function () {
      testCases.valid.forEach(
        ({ config = {}, it: _it, moddleElement, name }, idx) =>
          (_it || it)(getTitle(idx, name), function () {
            return Promise.resolve(moddleElement)
              .then((moddleRoot) => {
    ...
    describe("should lint invalid", function () {
      testCases.invalid.forEach(
        ({ config = {}, it: _it, moddleElement, name, report }, idx) =>
          (_it || it)(getTitle(idx, name), async function () {
            if (!Array.isArray(report)) {
              report = [report];
            }

            const { meta } = await linter.resolveRule(name, config);
...
export function getTitle(idx, name) {
...
export default { expectEqual, verify, getTitle, replacer };
